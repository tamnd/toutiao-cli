...
describes what users get. Pick a format with `-o`, or let toutiao choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
toutiao <command> -o table   # aligned columns for reading
toutiao <command> -o jsonl   # one JSON object per line, for piping
toutiao <command> -o json    # a single JSON array
toutiao <command> -o csv     # spreadsheet friendly
toutiao <command> -o tsv     # tab-separated
toutiao <command> -o url     # just the URL column
toutiao <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
toutiao <command> --fields id,title,url
```
...
```bash
toutiao <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
toutiao <command>            # a table, because this is a terminal
toutiao <command> | wc -l    # JSONL, because this is a pipe
```
